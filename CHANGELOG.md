...
- **Reflow при ресайзе**: ядро симулирует поведение терминала при изменении размера (включая reflow по ширине), синхронизируя буфер и устраняя артефакты.
...
### Changed
- **Размер кода**: 10050 → 9589 строк (−4.6%).
...
### Performance
- **FPS на `examples/dashboard`**: ~200 с I/O → **~1500–1800**, **~2500–3000** (без I/O). Ускорение **x7–10** на реальном приложении с графиками, таблицами, вкладками и анимациями.