...
### Fixed
- Исправлено отображение гиперссылок — убраны отступы по умолчанию.
- Исправлена работа `SetStyleFunc`.
...
### Added
- **Новая архитектура рендеринга**: виджеты рисуют напрямую в `[][]cell.Cell` через `Render(buf)`, без промежуточных ANSI-строк. Убрано двойное преобразование `Cell → String → Cell`, что дало кратный прирост производительности.
...
### Fixed
- **Паника `index out of range` в `draw`**: добавлена проверка выхода виджета за границы буфера по вертикали и горизонтали. Раньше при недостатке места в терминале (например, после сжатия) ядро пыталось записать ячейку за пределами `buf`.
- Баг установки текущей вкладки `Tabs`.
